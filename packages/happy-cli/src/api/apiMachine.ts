...
type MachineRpcHandlers = {
    ...
    resumeSession?: (sessionId: string, options?: { model?: string; permissionMode?: string; token?: string }) => Promise<SpawnSessionResult>;
...
export class ApiMachineClient {
    ...
    private resumeSessionHandler: ((sessionId: string, options?: { model?: string; permissionMode?: string; token?: string }) => Promise<SpawnSessionResult>) | null = null;
        ...
        if (this.resumeSessionHandler) {
            if (!this.rpcHandlerManager.hasHandler(method)) {
                this.rpcHandlerManager.registerHandler(method, async (params: any) => {
                    const { sessionId, model, permissionMode, token } = params || {};
                    ...
                    const result = await handler(sessionId, { model, permissionMode, token });
                    switch (result.type) {
                        case 'success':
                            return { type: 'success', sessionId: result.sessionId };
                        case 'requestToApproveDirectoryCreation':
